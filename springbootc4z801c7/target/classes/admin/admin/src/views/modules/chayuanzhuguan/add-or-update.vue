...
export default {
	data() {
		...
		return {
			...
			rules: {
				zhuguanzhanghao: [
					{ required: true, message: '主管账号不能为空', trigger: 'blur' },
				],
				mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chayuanzhuguan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chayuanzhuguanCrossAddOrUpdateFlag = false;
						 this.parent.search();
